...
- **Write tests for coverage and important edge cases only** - avoid redundant tests that don't add value
- **Simple enums**: One test verifying all expected values exist is sufficient
- **DTOs/POJOs**: Only test custom logic; don't test getters/setters generated by Lombok, unless jacoco test coverage calls for it. 
- **Don't test Java language features**: No need to test `valueOf()`, `name()`, `ordinal()`, switch statements, etc.
- **Focus on business logic**: Test the code YOU write, according to how the test is intended to behave, not framework or language behaviour
- **One assertion per concept**: Multiple assertions in one test are fine if testing a single logical concept
